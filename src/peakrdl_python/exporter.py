    ...
    def _build_node_type_table(self, node: AddressableNode,
        ...
        for child_node in get_dependent_component(node.parent, hide_node_func):
            ...
            if cand_type_name in self.node_type_name.values():
                if child_node == node:
                    raise RuntimeError(f'Top Node name {cand_type_name} already used by instance {list(self.node_type_name.keys())[list(self.node_type_name.values()).index(cand_type_name)]}')
